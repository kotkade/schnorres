(ns kotka.test-schnorres
  (:require
    [kotka.schnorres :refer :all]
    [clojure.test :refer [deftest is are]]
    [lamina.core :as lamina]))

(def hello
  (respond-constantly {:status  200
...
(defn request
  [handler uri & options]
  (run-request handler (conj {:request-method :get :uri uri}
...
(deftest wrapping
  ...
  (is (found+content (app (fn [chan req] (hello chan req))) "/random/url"))
  (is (found+content (app :handler (fn [chan req] (hello chan req))) "/random/url")))
...
(deftest params
  (is (= {:body "bar/bat%"}
         (run-request (app {:keys [foo biz]} {:body (str foo "/" biz)})
                      {:query-string "foo=bar&biz=bat%25"})))
  (is (= {:body "/"}
         (run-request (app {:keys [foo biz]}
                           {:body (str foo "/" biz)})
                      {:query-string "foo=bar&biz=bat%25" :params {}})))
  (is (= {:body "bar/bat%"}
         (run-request (app :params   [foo biz]
                           :response {:body (str foo "/" biz)})
                      {:query-string "foo=bar&biz=bat%25"})))
  (is (= {:body "/"}
         (run-request (app :params   [foo biz]
                           :response {:body (str foo "/" biz)})
